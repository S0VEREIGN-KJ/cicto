...
?><!--Back to office List button-->
<a href="offices.php" style="float: left; margin-left: 300px; margin-top: 20px;">
...
<div style="width: 100%; margin-left: 100px; text-align: center;margin-bottom: 50px; margin-top: 50px;">   <!--FULL TABLE CSS-->
...
<br>
  ...
  <table border="3" style="border-collapse: collapse;width: 80%;font-size: 20px; line-height: 24px; margin: 0 auto;"> <!--TABLE-->
  <tr>
    <th>Ticket #</th>
    <th>Serial #</th>
...
<!-- DATE TABLE -->
    <th>
  <?php
  echo 'Date/Time Requested ';
  ?>
  <select name="date_req_sort" onchange="location.href=this.value">
...
<?php
  echo 'Priority ';
  ?>
  <select name="priority_sort" onchange="location.href=this.value">
    <option value="<?php echo $_SERVER['PHP_SELF']; ?>?sort=priority&order=ASC" <?php if ($sort == 'priority' && $order == 'ASC') echo 'selected'; ?>>Low, Medium, High</option>
    <option value="<?php echo $_SERVER['PHP_SELF']; ?>?sort=priority&order=DESC" <?php if ($sort == 'priority' && $order == 'DESC') echo 'selected'; ?>>High, Medium, Low</option>
  </select>
</th>
<!--END OF PRIORITY SELECTION TABLE -->
...
<!-- END STATUS SELECTION TABLE -->


  <!-- GET DATA'S ON DATABASE -->
  <?php   
if ($sort == 'datetime_req') {
  $sql = "SELECT * FROM ticket WHERE office = '$office' ORDER BY datetime_req " . ($order == 'ASC' ? 'ASC' : 'DESC');
} elseif ($sort == 'priority') {
  $sql = "SELECT * FROM ticket WHERE office = '$office' ORDER BY FIELD(priority, 'Low', 'Medium', 'High') " . ($order == 'ASC' ? '' : 'DESC');
} elseif ($sort == 'status') {
  $sql = "SELECT * FROM ticket WHERE office = '$office' ORDER BY FIELD(status, 'Pending', 'In Progress', 'Closed') " . ($order == 'ASC' ? '' : 'DESC');
} else {
  $sql = "SELECT * FROM ticket WHERE office = '$office' ORDER BY datetime_req DESC";
  $order = 'ASC';
}
    ...
    if ($result->num_rows > 0) {
      while($row = $result->fetch_assoc()) {
        $ticket_number = $row["ticket_number"]; // Define $ticket_number here
$comment = $row['comment'];
...
$status = $row['status'];
      ...
      $priority = $row["priority"];
        ...
        echo '<td><a href="#" onclick="openNav(' . $row["ticket_number"] . ')">Edit</a></td>'; // Details option click
      
      
echo "</tr>";// Details option click// Details option click// Details option click// Details option click// Details option click
      }
    } else {
...
<form action="update_office.php" method="post" autocomplete="off">
...
<div class="overlay_form_group">
   ...
   <div class="overlay_form_group">
          ...
          <div class="overlay_form_group">
              ...
              <input type="date" id="item_date_received" name="item_date_received" value="<?php echo $item_date_received; ?>">
          </div>

          <div class="overlay_form_group">
              <label for="received_by">Received by:</label><!-- received_by -->
              <input type="text" id="received_by" name="received_by" value="<?php echo $received_by; ?>">
          </div>

          <div class="overlay_form_group">
              <label for="assigned_name">Name of Technician:</label>
              <input type="text" id="assigned_name" name="assigned_name" value="<?php echo $assigned_name; ?>"><!-- assigned_name -->
          </div>

...
if ($conn->connect_error) {
  ...
  $search_ticket_number = isset($_GET['search_ticket_number']) ? $_GET['search_ticket_number'] : '';
  $search_serial_number = isset($_GET['search_serial_number']) ? $_GET['search_serial_number'] : '';
  $search_req_name = isset($_GET['search_req_name']) ? $_GET['search_req_name'] : '';
  $search_unit = isset($_GET['search_unit']) ? $_GET['search_unit'] : '';
  $search_category = isset($_GET['search_category']) ? $_GET['search_category'] : '';
  $search_assigned_name = isset($_GET['search_assigned_name']) ? $_GET['search_assigned_name'] : '';

  // Define default sorting and ordering
  $sort = 'datetime_req';
  $order = 'DESC';
...
// Define search query
$search_query = "SELECT * FROM ticket WHERE status != 'Pending'";

// Add search conditions
if (!empty($search_input)) {
  $conditions = array();
  $params = array();
  $conditions[] = "ticket_number LIKE ?";
  $params[] = "%$search_input%";
  $conditions[] = "serial_number LIKE ?";
  $params[] = "%$search_input%";
  $conditions[] = "req_name LIKE ?";
  $params[] = "%$search_input%";
  $conditions[] = "unit LIKE ?";
  $params[] = "%$search_input%";
  $conditions[] = "category LIKE ?";
  $params[] = "%$search_input%";
  $conditions[] = "assigned_name LIKE ?";
  $params[] = "%$search_input%";
  
  $search_query .= " AND (" . implode(" OR ", $conditions) . ")";
}
    ...
    if ($result->num_rows > 0) {
      while($row = $result->fetch_assoc()) {
        ...
        echo "<td>" . $row["assigned_name"] . "</td>";
              ...
              echo '<td><a href="#" onclick="openNav(' . $row["ticket_number"] . ')">View/Print</a></td>'; // Details option click

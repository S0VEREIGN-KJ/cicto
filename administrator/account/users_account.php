...
<a href="closed_accounts.php" style="float: right; margin-right: 100px; margin-top: 20px; text-decoration: none;">
    <button class="arrow-button" style="background-color:#AA0000; color: #fff; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; display: flex; align-items: center; gap: 8px; font-weight: bold;">
        Closed Account List &rarr;
    </button>
</a>
...
">
  ...
  AND office NOT IN (
      ...
      FROM account
      WHERE account_activation_hash IS NOT NULL
  )  -- Exclude offices where any user has a non-NULL account_activation_hash
  GROUP BY office
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    ?>
     <a href="show_user.php?office=<?=$office?>">
